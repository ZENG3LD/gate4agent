...
use gate4agent_c2::protocol::{
    C2RelayFailureCode, NodeId, NodeRoute, NodeTransportState,
};
use gate4agent_c2_client::{connect_local, C2Client, C2ControlError};
use gate4agent_node::protocol::{
    AgentProvider, ClientRole, NodeEvent, NodeRequest, NodeResponse, SessionMode, WorkspaceId,
};
...
async fn windows_real_two_node_c2_control_relay_routes_commands_events_and_preserves_nodes_on_exit() {
    ...
    let event_drain = tokio::spawn(async move {
        ...
        let mut last_a_sequence = Some(hello_a_cursor.sequence);
        let mut saw_added = false;
        while let Some(event) = events.recv().await {
            count += 1;
            if event.node_id == event_node_a {
                assert_eq!(event.cursor.incarnation_id, hello_a_cursor.incarnation_id);
                if let Some(previous) = last_a_sequence {
                    assert!(event.cursor.sequence > previous, "node A events were not monotonic");
                }
                last_a_sequence = Some(event.cursor.sequence);
                if matches!(event.event, NodeEvent::WorkspaceAdded { ref workspace }
    ...
    let route_a = route(&initial, &id_a);
    ...
    assert!(matches!(snapshot_a.unwrap().response, Ok(NodeResponse::Snapshot { .. })));
    assert!(matches!(snapshot_b.unwrap().response, Ok(NodeResponse::Snapshot { .. })));
    ...
    assert!(matches!(inspected.response, Ok(NodeResponse::WorkspaceInspected { .. })));
    ...
    assert!(matches!(added.response, Ok(NodeResponse::WorkspaceRegistered { .. })));
    ...
    let session = match spawned.response {
        Ok(NodeResponse::SpawnAccepted { session }) => session,
    ...
    timeout(Duration::from_secs(5), async {
        loop {
            ...
            if matches!(response.response, Ok(NodeResponse::Accepted)) { break; }
            sleep(Duration::from_millis(20)).await;
        }
    ...
    }).await.unwrap().response, Ok(NodeResponse::Accepted)));
    ...
    drop(control);
    let (event_count, saw_added) = timeout(Duration::from_secs(2), event_drain).await.unwrap().unwrap();
    ...
    assert!(matches!(post_exit, NodeResponse::Snapshot { ref snapshot, .. }
        if snapshot.workspaces.iter().flat_map(|workspace| &workspace.sessions).any(|item| item.instance_id == session.session.instance_id)));
    ...
    assert_eq!(direct_a.request(NodeRequest::Stop { session, force: true }).await.unwrap(), NodeResponse::Accepted);
    drop(direct_a);
