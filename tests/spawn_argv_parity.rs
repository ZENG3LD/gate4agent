...
// ─────────────────────────────────────────────
// OpenClaw — Phase 4 real implementation
// ─────────────────────────────────────────────
//
// Argv shape (fresh): acpx openclaw --format json "<prompt>"
// Argv shape (resume): acpx openclaw --format json --session <id> "<prompt>"
//
// Source: docs/research/cli-agents-headless-modes-2026.md, OpenClaw section.
// Assumed per ACP client convention; verify against live acpx install.

#[test]
fn openclaw_fresh_session_argv() {
    let builder = cli_builder(CliTool::OpenClaw);
    let opts = make_opts("do something useful");
    let cmd = builder.build_command(&opts);

    assert_eq!(get_program(&cmd), "acpx", "OpenClaw client binary must be 'acpx'");

    let got = get_args(&cmd);

    // Must start with openclaw subcommand
    assert_eq!(got.first().copied(), Some("openclaw"), "first arg must be 'openclaw'");

    // Must include --format json
    assert!(
        got.windows(2).any(|w| w == ["--format", "json"]),
        "argv must include '--format json'"
    );
    ...
    assert_eq!(
        ...
        "OpenClaw: prompt must be the last argv token"
    );

    // Must NOT include --session when no resume_session_id
    assert!(
        !got.contains(&"--session"),
        "fresh session must not include --session"
    );
}

#[test]
fn openclaw_with_session_argv() {
    let builder = cli_builder(CliTool::OpenClaw);
    let opts = SpawnOptions {
        prompt: "continue the task".to_string(),
        resume_session_id: Some("claw_ses_abc".to_string()),
        ..Default::default()
    };
    let cmd = builder.build_command(&opts);

    assert_eq!(get_program(&cmd), "acpx");

    let got = get_args(&cmd);

    // --session <id> must appear before the prompt
    let session_pos = got.iter().position(|a| *a == "--session");
    assert!(session_pos.is_some(), "--session flag must appear in resume argv");

    let id_pos = session_pos.unwrap() + 1;
    assert_eq!(
        got.get(id_pos).copied(),
        Some("claw_ses_abc"),
        "--session value must be the session ID"
    );
    ...
    assert_eq!(
        got.last().copied(),
        Some("continue the task"),
        "OpenClaw resume: prompt must be the last argv token"
    );

    // --session must appear before the prompt
    let prompt_pos = got.iter().position(|a| *a == "continue the task").unwrap();
    assert!(
        session_pos.unwrap() < prompt_pos,
        "--session must appear before the prompt in argv"
    );
}
